...
console.log("[Ticket] Sistema de Tickets (V15 - Multi-Category Routing) carregado!");
function getGuildImage(guild) {
    return (guild?.iconURL?.({ size: 1024 }) ||
        guild?.bannerURL?.({ size: 1024 }) ||
        "https://cdn.discordapp.com/embed/avatars/0.png");
}
function getClientName(user, member) {
    return member?.displayName || user?.globalName || user?.username || "Cliente";
}
// Função compartilhada para criar o ticket
function createTicketButtons(closeStyle = ButtonStyle.Secondary) {
    return createRow(new ButtonBuilder({
