...
import { ButtonBuilder, ButtonStyle, UserSelectMenuBuilder, TextInputBuilder, TextInputStyle, ModalBuilder, LabelBuilder, PermissionFlagsBits, RadioGroupBuilder, StringSelectMenuBuilder, } from "discord.js";
...
function formatTranscriptDuration(start, end) {
...
function getTranscriptContent(message) {
    ...
    for (const embed of message.embeds) {
        if (embed.title)
            parts.push(`[Embed] ${embed.title}`);
        if (embed.description)
            parts.push(embed.description);
    }
    return parts.filter(Boolean).join("\n") || "[Mensagem sem texto]";
}
function resolveTranscriptUrl(url) {
    if (/^https?:\/\//i.test(url))
        return url;
    return `${env.WEB_URL.replace(/\/$/, "")}/${url.replace(/^\//, "")}`;
}
async function sendTranscriptToSite(payload) {
    const headers = {
    ...
    if (!response.ok) {
        throw new Error(`API de transcripts respondeu ${response.status}: ${JSON.stringify(data)}`);
    }
...
async function saveTranscriptToSharedDatabase(_transcriptData) {
    return;
}
const DEFAULT_TICKET_STATUS_TITLE = "ATIVO";
const DEFAULT_TICKET_STATUS_DESCRIPTION = "O player está ativo.";
...
function getConfiguredTicketCategoryOptions(configured) {
...
function getTicketStatusOption(value) {
    return TICKET_STATUS_OPTIONS.find((status) => status.value === value);
}
function getAdminStatusText(ticket) {
...
async function replyTicketNeedsStaffRole(interaction) {
    const response = {
        content: "<:action_warning:1502789801949265990> Você não tem permissão para usar essa ação.",
        flags: ["Ephemeral"],
    };
...
async function replyTicketNeedsClaimer(interaction) {
    const response = {
...
async function updateMainTicketMessage(channel, ticket, guild) {
    if (!ticket.messageId || !channel?.isTextBased())
        return;
...
function createMainPanel(ticket, owner, guild) {
    ...
    }), ...statusBlocks, Separator.Default, `<:foldersearch:1520843134521577615> **Categoria do atendimento:**\n\`\`\`\n${ticket.category.toUpperCase()}\n\`\`\``, `<:bagdinfo:1520843355108544683> **Motivo do contato:**\n\`\`\`\n${ticket.description}\n\`\`\``, Separator.Default, createRow(...(isClaimed
        ? []
        : [
            new ButtonBuilder({
    ...
    async run(interaction, { action }) {
        ...
        switch (action) {
            case "claim": {
                ...
                if (owner) {
                    const dmContainer = createContainer(constants.colors.white, createSection({
            ...
            case "admin": {
                ...
                const container = createContainer("#00FFD4", createSection({
            ...
            case "transfer": {
                ...
                const options = getConfiguredTicketCategoryOptions(guildData.channels?.categories || {});
                if (options.length === 0) {
                    await interaction.reply({
            ...
            case "members_modal": {
                ...
                const label = new LabelBuilder()
                    ...
                    .setDescription("Selecione o usuário que deseja adicionar ou remover.")
            ...
            case "close": {
                ...
                await interaction.reply({
                    content: "Por favor, use o botÃ£o de finalizar para abrir o formulÃ¡rio.",
                    flags: ["Ephemeral"],
                });
                break;
            }
            case "delete": {
                ...
                const transcript = await generateTranscript(channel, ticket, user);
...
createResponder({
    ...
    async run(interaction) {
        ...
        if (categories) {
            parentId = categories[newCategory];
        }
        if (!parentId) {
            await interaction.editReply({
...
createResponder({
    ...
    async run(interaction) {
        ...
        if (!ticket) {
            await interaction.editReply({
                components: [
                    createContainer(constants.colors.danger, "<:action_warning:1502789801949265990> Ticket não encontrado."),
                ],
            });
            return;
        }
        ...
        if (!selectedStatus) {
            await interaction.editReply({
                components: [
                    createContainer(constants.colors.danger, "<:action_warning:1502789801949265990> Status inválido."),
                ],
            });
            return;
        }
        ...
        await interaction.editReply({
            components: [
                createContainer(constants.colors.white, `<:check:1520842193257103532> Status do player atualizado para **${selectedStatus.label}**.`),
            ],
        });
    },
});
export async function generateTranscript(channel, ticket, closer) {
    ...
    const sortedMessages = [...messages.values()].sort((a, b) => a.createdTimestamp - b.createdTimestamp).filter((msg) => !msg.author.bot);
    const openedAt = getTicketOpenedAt(ticket);
    const closedAt = new Date();
    const fallbackCode = ticket.ticketId || Math.random().toString(36).substring(2, 9).toUpperCase();
    const claimer = ticket.claimedBy
        ? await channel.guild.members.fetch(ticket.claimedBy).catch(() => null)
        : null;
    const agentName = claimer?.displayName ||
        closer.displayName ||
        closer.globalName ||
        closer.username ||
        "Equipe";
    const transcriptPayload = {
        ...
        messages: sortedMessages.map((msg) => {
            const isAgent = msg.member?.permissions.has(PermissionFlagsBits.ManageChannels) ||
                msg.author.bot ||
                false;
            return {
    ...
    await db.transcripts.updateOne({ id: fallbackCode }, { $set: localTranscript }, { upsert: true });
    try {
        const transcript = await sendTranscriptToSite(transcriptPayload);
        await db.transcripts.updateOne({ id: transcript.id }, {
            $set: {
                ...localTranscript,
                id: transcript.id,
                code: transcript.id,
        ...
        console.error("[Transcript] Nao foi possivel enviar para a API do site. O transcript ficou salvo localmente:", error);
        return {
            id: fallbackCode,
    ...
    await saveTranscriptToSharedDatabase(transcriptData).catch((error) => {
        console.error("[Transcript] Nao foi possivel salvar no MongoDB compartilhado. O transcript ficou salvo localmente:", error);
    });
    return {
        id: transcriptId,
