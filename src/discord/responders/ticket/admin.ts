  ...
  try {
    ...
    if (!(await hasTicketStaffPermission(interaction))) {
      await interaction
        .editReply({
          content:
            "<:action_warning:1502789801949265990> Você não tem permissão para finalizar este ticket.",
          components: [],
        })
        .catch(() => {});
      return;
...
Venho registrar a log de encerramento do atendimento \`${ticket.ticketId}\`, encerrado por ${user}. O transcript nao foi salvo por escolha da staff.`,
          ...
          Separator.Default,
          `**<:check:1520842193257103532> Considerações Finais:**\n\`\`\`\n${considerations}\n\`\`\``,
    ...
    if (ownerMember) {
      ...
      const dmContainer = createContainer(
        ...
        `<:calendar:1502789854486986752> **Encerrado em:** <t:${closeTime}:f>`,
        Separator.Default,
        `<:check:1520842193257103532> **Considerações Finais:**\n\`\`\`\n${considerations}\n\`\`\``,
