...
function setOptionalInputValue(
  ...
  if (value) input.setValue(limitInputValue(value, maxLength));
  return input;
}
...
function formatTicketLogChannel(channelId?: string) {
...
function createPanelConfigContainer(guildData: any) {
  ...
  return createContainer(
    ...
    `**Canal de Logs:** ${formatTicketLogChannel(logChannelId)}`,
...
function createCategoryId(label: string, categories: RegisterCategory[]) {
  const base =
    label
      .normalize("NFD")
      .replace(/[\u0300-\u036f]/g, "")
...
function createRegistrationConfigContainer(config: ReturnType<typeof getRegistrationConfig>) {
  ...
  return createContainer(
    ...
    Separator.Default,
    `**Cargo Inicial:** <@&${config.initialRoleId}>`,
    Separator.Default,
    `**Categorias de Set:**\n${categories}`,
    Separator.Default,
    createRow(
...
function createCategoryModal(customId: string, category?: RegisterCategory) {
...
function createCategorySelect(customId: string, categories: RegisterCategory[], placeholder: string) {
  return createRow(
...
function readCategoryFields(data: Record<string, unknown>) {
    ...
    await interaction.reply({
      ...
      components: [
        createCategorySelect(
          "config/registro/category/edit_select",
          config.categories,
...
createResponder({
  ...
  async run(interaction) {
    ...
    await interaction.reply({
      ...
      components: [
        createCategorySelect(
          "config/registro/category/delete_select",
          config.categories,
          "Escolha uma categoria...",
        ),
      ],
