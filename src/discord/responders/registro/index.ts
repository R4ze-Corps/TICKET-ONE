...
type RegisterRequest = {
  ...
  categoryId: string;
  categoryType: RegisterCategory["type"];
  roleId: string;
  roleLabel: string;
  fullName: string;
  playerId: string;
...
const pendingRequests = new Map<string, RegisterRequest>();
const roleMessagesByUser = new Map<string, string>();

function createRequestId() {
  return Math.random().toString(36).slice(2, 10).toUpperCase();
}

function normalizeCategoryType(value?: string): RegisterCategory["type"] {
  return value?.toLowerCase().includes("respons") ? "responsavel" : "player";
}

function getRegisterCategoryEmoji(category: any) {
  const key = `${category?.id || ""} ${category?.label || ""}`.toLowerCase();
  if (key.includes("respons")) return "1520828253940486206";
  if (key.includes("kid") || key.includes("player")) return "1520826742972088371";
  return category?.emoji ? String(category.emoji) : undefined;
}

function getRegistrationConfig(guildData: any) {
  const saved = guildData.registration || {};
  const savedCategories = Array.isArray(saved.categories)
    ? saved.categories.filter((category: any) => category?.id && category?.label && category?.roleId)
    : [];

  return {
    initialRoleId: saved.initialRoleId || DEFAULT_INITIAL_ROLE_ID,
    categories: (savedCategories.length > 0 ? savedCategories : DEFAULT_REGISTER_CATEGORIES).map(
      (category: any) => ({
...
function getSingleField(data: Record<string, unknown>, key: string) {
  const value = data[key];
  return String(Array.isArray(value) ? value[0] || "" : value || "").trim();
}

function createApprovedNickname(request: RegisterRequest) {
  const nickname = `#${request.playerId} ${request.fullName}`.replace(/\s+/g, " ").trim();
  return nickname.slice(0, 32);
}

function formatUsers(userIds: string[]) {
  return userIds.map((id) => `<@${id}>`).join(", ");
}

function getRelatedLabel(request: RegisterRequest) {
  return request.categoryType === "player"
    ? "Pais responsaveis"
    : "Criancas responsaveis";
}

function formatRequestSummary(request: RegisterRequest) {
  const lines = [
...
async function deleteMessageById(interaction: any, messageId?: string) {
...
async function getCategory(interaction: any, categoryId: string) {
...
async function createRoleSelectContainer(interaction: any) {
  const guildData = await db.guilds.get(interaction.guildId!);
  const config = getRegistrationConfig(guildData);

  return createContainer(
    constants.colors.white,
    "## \u{1F4DD} Formulario de Registro",
    Separator.Default,
    "Primeiro, O que voce e?",
    createRow(
...
function createRegisterModal(category: RegisterCategory) {
...
function createUserSelectContainer(request: RegisterRequest) {
  const prompt =
    request.categoryType === "player"
...
function createConfirmContainer(request: RegisterRequest) {
  return createContainer(
    constants.colors.white,
    "## \u{1F4DD} Formulario de Registro",
    Separator.Default,
    formatRequestSummary(request),
    Separator.Default,
    "Confira suas informacoes e escolha uma opcao abaixo.",
    createRow(
      new ButtonBuilder({
...
function createReviewContainer(request: RegisterRequest, status?: string) {
...
createResponder({
  ...
  async run(interaction, { requestId }) {
    ...
    if (!request || request.applicantId !== interaction.user.id) {
      await interaction.reply({
        content:
          "Nao encontrei este registro. Comece novamente pelo botao Registrar.",
        flags: ["Ephemeral"],
      });
      return;
    }
...
createResponder({
  ...
  async run(interaction, { requestId }) {
    ...
    if (!channel?.isTextBased()) {
      await interaction.reply({
        content:
          "Nao foi possivel encontrar o canal de analise de registros.",
        flags: ["Ephemeral"],
      });
      return;
    }
  ...
  async run(interaction, { action, requestId }) {
    ...
    if (action === "approve") {
      ...
      await member.roles.add(request.roleId).catch(async () => {
        await interaction.reply({
          content:
            "Nao consegui adicionar o cargo. Verifique as permissoes e hierarquia do bot.",
          flags: ["Ephemeral"],
        });
      });
      ...
      await member.roles.remove(config.initialRoleId).catch(async () => {
        await interaction.reply({
          content:
            "Cargo escolhido adicionado, mas nao consegui remover o cargo inicial. Verifique as permissoes e hierarquia do bot.",
          flags: ["Ephemeral"],
        });
      });
      ...
      await member.setNickname(nickname).catch(async () => {
        await interaction.reply({
          content:
            "Cargo adicionado, mas nao consegui alterar o nome do player. Verifique as permissoes e hierarquia do bot.",
          flags: ["Ephemeral"],
        });
      });
      ...
      await interaction.update({
        components: [
          createReviewContainer(
            request,
            `<:check:1520842193257103532> **Aprovado por:** ${interaction.user}`,
          ),
        ],
      });
      return;
    ...
    if (action === "reject") {
      ...
      await interaction.update({
        components: [
          createReviewContainer(
            request,
            `<:action_remove:1502789800967536741> **Recusado por:** ${interaction.user}`,
          ),
        ],
      });
      return;
